...
def model(params, config, debug_mode=False):
    ...
    image_bg = image + logistic_background(params.background_slope, params.background_angle, params.background_translation)
    image = jnp.where(debug_mode, image, image_bg)

    image_conv = jax.scipy.signal.convolve2d(image, params.kernel, mode='same', boundary='fill')
    image = jnp.where(debug_mode, image, image_conv)
...
def get_init_knots(init_scale, config, prng_key = None):
    ...
    init_knots = random.uniform(prng_key, (config.n_splines, config.s_knots, 2))
    init_knots = jnp.stack([init_knots, init_scale * jnp.ones((config.n_splines, config.s_knots, 2))], axis=2)
...
def fit(image, 
    ...
    config = {
        ...
        "n_splines": init_knots.shape[1], # the number of splines
